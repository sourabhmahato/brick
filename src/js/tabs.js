...
import { BkBase } from './base.js';

class BkTabs extends BkBase {
  #tabs = [];
  #panels = [];
  ...
  init() {
    ...
    if (this.#tabs.length === 0 || this.#panels.length === 0) {
      console.warn('bk-tabs: Missing tab or tabpanel elements');
      return;
    }
    ...
    this.#tabs.forEach((tab, i) => {
      ...
      const tabId = tab.id || `bk-tab-${this.uid()}`;
      const panelId = panel.id || `bk-panel-${this.uid()}`;
  ...
  #activate(idx) {
    ...
    this.emit('bk-tab-change', { index: idx, tab: this.#tabs[idx] });
  }
...
customElements.define('bk-tabs', BkTabs);
